...
struct Chip8
{
    ...
    #define LIST_INSTRUCTIONS(o) \
        ...
        o("add vX,vY",   "8xy4", u==0x8 &&  p==0x4 , p = Vx+Vy;  Vx = p; VF =  (p>>8);) /* set Vx = Vx + Vy, update VF = carry */ \
        o("sub vX,vY",   "8xy5", u==0x8 &&  p==0x5 , p = Vx-Vy;  Vx = p; VF = !(p>>8);) /* set Vx = Vx - Vy, update VF = NOT borrow */ \
        o("subn vX,vY",  "8xy7", u==0x8 &&  p==0x7 , p = Vy-Vx;  Vx = p; VF = !(p>>8);) /* set Vx = Vy - Vx, update VF = NOT borrow */ \
        o("shr vX",      "8xy6", u==0x8 &&  p==0x6 , p = Vy  & 1;  Vx = Vy >> 1; VF = p    ) /* set Vx = Vy >> 1, update VF = carry */ \
        o("shl vX",      "8xyE", u==0x8 &&  p==0xE , p = Vy >> 7;  Vx = Vy << 1; VF = p    ) /* set Vx = Vy << 1, update VF = carry */ \
