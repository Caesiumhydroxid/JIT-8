...
// 00E0 - Clear the display.
    void
    CLS(const Hardware &cpu, asmjit::x86::Compiler &cc)
{
...
// 00EE - Return from a subroutine.
    void RET(const Hardware &cpu, BasicBlock *basicBlock, asmjit::x86::Compiler &cc, asmjit::x86::Gp cpubase,
             const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers)
{
    ...
    auto stackPointer = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, sp));
...
void JMP(Opcode instr, BasicBlock* basicBlock ,asmjit::x86::Compiler &cc,asmjit::x86::Gp cpubase, const std::array<asmjit::x86::Gp,Hardware::AMOUNT_REGISTERS> &registers)
{
...
void CALL(Opcode instr, const Hardware &cpu, BasicBlock* basicBlock, uint16_t pc ,asmjit::x86::Compiler &cc,asmjit::x86::Gp cpubase, const std::array<asmjit::x86::Gp,Hardware::AMOUNT_REGISTERS> &registers)
{
    auto ptr = cc.newUIntPtr();
    auto stackPointer = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, sp));
    auto stackMem = cc.newUIntPtr();
...
// Annn - Register I is set to nnn.
    ...
    auto memreg = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, indexRegister));
...
// Cxkk - Set Vx = random byte AND kk
    void RND(Opcode instr, asmjit::x86::Gp cpubase, asmjit::x86::Compiler &cc,
             const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
    auto lfsrReg = asmjit::x86::ptr_32(cpubase,offsetof(Hardware, rng));
    auto b = cc.newUInt32();
    cc.mov(b,lfsrReg);
    cc.and_(b,1);
    cc.neg(b);
...
void DRW(Opcode instr, const Hardware &cpu,asmjit::x86::Gp cpubase, c8::Memory &mem,asmjit::x86::Compiler &cc, const std::array<asmjit::x86::Gp,Hardware::AMOUNT_REGISTERS> &registers) {
    auto simI = cc.newUIntPtr();
    auto simDisp = cc.newUIntPtr();
    auto loopCounter = cc.newUInt64();
    auto indexRegisterValue = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, indexRegister));
...
// Ex9E - Skip instruction if key with the value of Vx is pressed.
    std::optional<asmjit::Label> SKP(Opcode instr, const Hardware &cpu, asmjit::x86::Compiler &cc,
                                     const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
...
// ExA1 - Skip instruction if key with the value of Vx is not pressed.
    std::optional<asmjit::Label> SKNP(Opcode instr, const Hardware &cpu, asmjit::x86::Compiler &cc,
                                      const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
    auto butAddr = cc.newUIntPtr();
    cc.mov(butAddr,reinterpret_cast<uint64_t>(cpu.buttons));
    auto tmp = cc.newUIntPtr();
...
// Fx07 - Set Vx = delay timer value.
    void LD_VX_DT(Opcode instr, const asmjit::x86::Gp& cpubase, asmjit::x86::Compiler &cc,
                  const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
    auto memreg = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, delayTimer));
    cc.mov(registers[instr.x()],memreg);
}

// Fx0A - Wait for a key press, store the value of the key in Vx.
    void LD_VX_K(Opcode instr, const Hardware &cpu, asmjit::x86::Compiler &cc,
                 const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
    auto butAddr = cc.newUIntPtr();
...
// Fx18 - Set sound timer = Vx.
    void LD_ST(Opcode instr, const asmjit::x86::Gp& cpubase, asmjit::x86::Compiler &cc,
               const std::array<asmjit::x86::Gp, Hardware::AMOUNT_REGISTERS> &registers) {
    auto memreg = asmjit::x86::ptr_16(cpubase,offsetof(Hardware, soundTimer));
    cc.mov(memreg,registers[instr.x()]);
}
